...
const s = StyleSheet.create({
  ...
  stripNum: { fontFamily: PDF_THEME.fontFamily, fontSize: 12, fontWeight: 700, marginBottom: 1 },
  ...
  funnelDesc: { fontFamily: PDF_THEME.fontFamily, fontSize: 6, color: PDF_THEME.textMuted, flex: 1, marginLeft: 4 },
  ...
  metricNum: { fontFamily: PDF_THEME.fontFamily, fontSize: 11, fontWeight: 700, marginBottom: 1 },
  metricLbl: { fontFamily: PDF_THEME.fontFamily, fontSize: 5.5, color: PDF_THEME.textMuted },
});

interface ImpactPageProps {
  timeLost: TimeLostBreakdown; leadsLost: LeadsLostData; dealerName: string;
  timeIntro: string; timeVerdict: string; leadsVerdict: string;
  pageNumber: number; totalPages: number;
}

export function ImpactPage({ timeLost, leadsLost, dealerName, timeIntro, timeVerdict, leadsVerdict, pageNumber, totalPages }: ImpactPageProps) {
  ...
  return (
    <Page size="A4" style={s.page}>
      ...
      <SectionTitle title="Impact Operationnel" subtitle={`Temps investi et opportunites perdues — ${timeLost.vehicleCount} vehicules`} />

      <Text style={s.introText}>{timeIntro}</Text>
      ...
      <View style={s.strip}>
        ...
        <View style={[s.stripItem, { backgroundColor: PDF_THEME.greenDim }]}>
          ...
          <Text style={s.stripLbl}>automatisables (IA)</Text>
        </View>
        <View style={s.stripItem}>
          <Text style={[s.stripNum, { color: PDF_THEME.textPrimary }]}>{timeLost.totalCostPerMonth.toLocaleString("fr-BE")} EUR</Text>
          <Text style={s.stripLbl}>cout mensuel (35 EUR/h)</Text>
        </View>
        <View style={[s.stripItem, { backgroundColor: PDF_THEME.redDim }]}>
          <Text style={[s.stripNum, { color: PDF_THEME.red }]}>{timeLost.totalCostPerYear.toLocaleString("fr-BE")} EUR</Text>
      ...
      <View style={s.tblHead}>
        ...
        <Text style={[s.tblTh, { flex: 3 }]}>Detail</Text>
      ...
      <Text style={s.sLabel}>Generation de leads</Text>
      ...
      <View style={s.funnelRow}>
        <Text style={s.funnelLbl}>Restants</Text>
        <View style={[s.funnelBar, { width: 170, backgroundColor: PDF_THEME.yellow }]}>
          <Text style={s.funnelVal}>{retained.toLocaleString("fr-BE")} visiteurs</Text>
        </View>
        <Text style={s.funnelDesc}>conversion ~2.5%</Text>
      </View>
      <View style={s.funnelRow}>
      ...
      <View style={s.metricsRow}>
        ...
        <View style={s.metricBox}><Text style={[s.metricNum, { color: PDF_THEME.red }]}>{leadsLost.revenueLostPerMonth.toLocaleString("fr-BE")} EUR</Text><Text style={s.metricLbl}>manque a gagner/mois</Text></View>
        <View style={s.metricBox}><Text style={[s.metricNum, { color: PDF_THEME.red }]}>{(leadsLost.revenueLostPerMonth * 12).toLocaleString("fr-BE")} EUR</Text><Text style={s.metricLbl}>projection annuelle</Text></View>
      </View>
